...
def mostrar_nomes_salarios(nomes, salarios):
    # Iterador zip
    for nome, salario in zip(nomes, salarios):
        print(f"Nome: {nome}, Salário: {salario}")

def calcular_custos_reajustados(salarios):
    # Iterador map
    salarios_reajustados = list(map(lambda salario: salario * 1.10, salarios))
    custo_total = sum(salarios_reajustados)
    return custo_total

def modificar_datas(datas):
    # Iterador filter
    datas_modificadas = list(filter(lambda data: data.ano < 2019, datas))
...
def main():
    ...
    # Iterador Zip
    print("\nIterador Zip:")
    mostrar_nomes_salarios(nomes.lista, salarios.lista)

    # Iterador Map
    print("\nIterador Map:")
    custo_folha_reajustada = calcular_custos_reajustados(salarios.lista)
    print(f"Custo total da folha de pagamento após reajuste: {custo_folha_reajustada}\n")

    # Iterador Filter
    print("\nIterador Filter:")
    datas_modificadas = modificar_datas(datas.lista)
    for data in datas_modificadas:
        print(data)
